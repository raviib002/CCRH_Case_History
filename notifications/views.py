...
from user_profile.models import (AdditionalProfile,
                                 PractCertificateUpload,
                                 PractDetails,Profilestatus)
...
#     paginate_by = settings.get_config()['PAGINATE_BY']
    def get_template_names(self):
            return ['notifications/backend_notifications_list.html']
...
class AllNotificationsList(NotificationViewList):
    ...
    def get_context_data(self, **kwargs):
        ...
        unread_notify = self.request.user.notifications.unread()
        read_notify = self.request.user.notifications.read()
        data['unread_notify'] = unread_notify
        data['read_notify'] = read_notify
        return data
...
"""To get the read notification list - starts"""
@login_required
def read_notifications_list(request):
    if request.method == "GET":
        print(request.user,'use')
        read_notify = request.user.notifications.read()
        print(read_notify,'read_notify')
    return render(request, "notifications/backend_notifications_list.html", {'read_notify':read_notify,
                                                                        'read_list':'read_list'})
"""To get the read notification list - ends"""
